...
$args = apply_filters('udesly/posts/website-assets-where-page-location-eq-about', $args);
...
<div class="w-dyn-list" udy-collection="website-assets">
            <?php if ( $query->have_posts() ) : ?><div role="list" class="w-dyn-items">
              <?php while ($query->have_posts()) : $query->the_post(); global $post; ?><div role="listitem" class="w-dyn-item">
                <div class="w-video w-embed" style="aspect-ratio: 16 / 9;"><?php echo wp_oembed_get(udesly_get_custom_post_field( $post->ID, "video-link-vimeo-youtube", "Video" )) ?></div>
              </div><?php endwhile; ?>
            </div>
